...
static int
hsuart_read(struct dev_ctxt* p_contxt, unsigned char *buf, int count)
{
	unsigned long		flags;
	int			ret;
	struct buffer_item*	p_buffer = NULL;
...
static int 
hsuart_ioctl(struct tty_struct *tty,
...
hsuart_close(struct tty_struct *tty, struct file *f)
{
	...
	if ( 0 != p_contxt->is_initilized ) {
		...
		if (ret) printk(KERN_ERR "%s: hsuart_uart_port_release ret=%d\n", __func__, ret);
